...
A Grafana dashboard ("Flux GitOps", uid `flux`) in the Homelab folder that
renders the Flux GitOps pipeline health — reconcile rate, errors, duration, and
per-resource churn — sourced from the series scraped under the `flux-metrics`
capability. Provisioned and reconciled by Terraform from
`terraform/grafana/dashboards/flux.json`, so the repo is the source of truth.
